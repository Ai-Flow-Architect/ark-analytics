...
GA4（example.com）
    ↓ 自動エクスポート（毎日）
BigQuery: __GA4_DATASET__.events_*
    ↓ 日次バッチ（毎朝4:00 cron）
...
BQプロジェクト `__ARK_PROJECT__` への `BigQueryデータ閲覧者` 権限が必要です。
...
2. example.com を開き、各タグが「Fired」になるか確認
...
FROM `__ARK_PROJECT__.marts.daily_kpi_summary`
ORDER BY report_date DESC
LIMIT 7;

-- ページ別パフォーマンス TOP10
SELECT page_path, SUM(pageviews) AS pv, ROUND(AVG(cta_click_rate)*100,1) AS cta_ctr
FROM `__ARK_PROJECT__.marts.page_performance`
GROUP BY page_path
ORDER BY pv DESC
LIMIT 10;
```
