...
GA4（example.invalid）
    ↓ 自動エクスポート（毎日）
BigQuery: analytics_REDACTED-GA4-PROPID.events_*
    ↓ 日次バッチ（毎朝4:00 cron）
...
```
0 4 * * * ~/projects/ark-analytics/scripts/daily_refresh.sh
```
...
```bash
# 今日のログを確認
cat ~/projects/ark-analytics/logs/refresh_$(date '+%Y%m%d').log

# 最新ログをリアルタイム確認
tail -f ~/projects/ark-analytics/logs/refresh_$(date '+%Y%m%d').log
```
...
2. Googleアカウント `REDACTED-CLIENT-EMAIL@example.invalid` でログイン
...
1. 「共有」→「他のユーザーと共有」
2. AIフローアーキテクト（`contact@ai-flow-architect.com`）に「閲覧者」権限を付与
3. 「リンクを知っている全員が閲覧可能」に設定（任意）

#### 権限エラーが出た場合

BQプロジェクト `REDACTED-GCP-PROJECT` への `BigQueryデータ閲覧者` 権限が必要です。
権限付与が必要な場合は、AIフローアーキテクト（contact@ai-flow-architect.com）にご連絡ください。
...
REDACTED-GTM-ID に計測タグを設置する手順です。
...
2. example.invalid を開き、各タグが「Fired」になるか確認
...
FROM `REDACTED-GCP-PROJECT.analytics_REDACTED-GA4-PROPID.events_*`
```
...
FROM `REDACTED-GCP-PROJECT.marts.daily_kpi_summary`
ORDER BY report_date DESC
LIMIT 7;

-- ページ別パフォーマンス TOP10
SELECT page_path, SUM(pageviews) AS pv, ROUND(AVG(cta_click_rate)*100,1) AS cta_ctr
FROM `REDACTED-GCP-PROJECT.marts.page_performance`
GROUP BY page_path
ORDER BY pv DESC
LIMIT 10;
```
