...
ark-hd.co.jp の GA4×BQ分析基盤で計測するカスタムイベントのGTMタグコードです。
...
> **GA4 イベントタグ**（測定ID `G-Y95H1GWBB7`）が担う GTM 正準パターンに変更。
>
...
- 計測先: **GA4 測定ID `G-Y95H1GWBB7`**（ストリームID 5511077682 / GTM-5B3L5372 経由）
...
- BQエクスポートにより `analytics_386840839.events_*` に約24-48時間後反映
...
### GA4 への送出
GA4イベントタグ `GA4 Event - Scroll Depth`（測定ID `G-Y95H1GWBB7` / イベント名 `scroll_depth` /
...
### GA4 への送出
GA4イベントタグ `GA4 Event - CTA Click`（測定ID `G-Y95H1GWBB7` / イベント名 `cta_click` /
...
### GA4 への送出
GA4イベントタグ `GA4 Event - Form`（測定ID `G-Y95H1GWBB7` / イベント名 `{{DLV - fe_form_action}}`
...
4. **GA4イベントタグ 3個** 作成（測定ID `G-Y95H1GWBB7` / 各カスタムイベントトリガーで発火）
