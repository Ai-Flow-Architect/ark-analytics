...
```
example.invalid（Webサイト）
  │
  ├── GTMタグ（REDACTED-GTM-ID）
  │     ├── タグ①: スクロール90%計測
  │     ├── タグ②: CTAクリック計測
  │     └── タグ③: フォーム送信計測
  │
  ▼
GA4（プロパティID: REDACTED-GA4-PROPID）
  │
  ▼  ← BigQuery Export（毎日自動）
BigQuery（REDACTED-GCP-PROJECT）
  ├── staging/  ← GA4生データ整形
  ├── marts/    ← 分析テーブル5種
  ...
  │         https://REDACTED-APP.example/
  │
  └──▶ 月次AIレポート（毎月1日自動メール）
            配信先: REDACTED-CLIENT-EMAIL@example.invalid
```
...
| # | 納品物 | 状態 | URL / 備考 |
|---|--------|------|-----------|
| ① | BigQuery 分析基盤 | ✅ 完成 | GCPプロジェクト: REDACTED-GCP-PROJECT |
| ② | 月次AI自動レポート | ✅ 完成・cron稼働中 | 毎月1日 AM9:00 自動配信 |
| ③ | Streamlit 自然言語QAアプリ | ✅ v2.2 本番稼働中 | https://REDACTED-APP.example/ |
| ④ | 改善施策スコアリング | ✅ 完成（Streamlitに統合） | QAアプリ内で動作 |
| ⑤ | Looker Studio ダッシュボード | ⚠️ 表示OK・日本語化が未完 | 後述「残課題①」参照 |
| ⑥ | GTM カスタムイベント3種 | ✅ 設置・確認済み | REDACTED-GTM-ID / 4/23確認 |
...
| システム | URL |
|---------|-----|
| Streamlit QAアプリ | https://REDACTED-APP.example/ |
...
| 項目 | 値 |
|------|---|
| GCPプロジェクトID | `REDACTED-GCP-PROJECT` |
| GA4プロパティID | `REDACTED-GA4-PROPID` |
| BigQuery データセット | `staging` / `marts` / `reports` |
| Looker Studio 共有先 | REDACTED-CLIENT-EMAIL@example.invalid（閲覧者権限付与済み） |

### GTM

| 項目 | 値 |
|------|---|
| GTMコンテナID | `REDACTED-GTM-ID` |
| 設置確認日 | 2026-04-23 |
| 設置確認方法 | `curl -s https://example.invalid` でスニペット確認済み |
...
| トークルーム | https://coconala.com/talkrooms/REDACTED-TALKROOM |
...
**URL:** https://REDACTED-APP.example/
...
| 変数名 | 用途 |
|-------|------|
| `ARK_CLIENT_EMAIL` | レポート配信先メール（REDACTED-CLIENT-EMAIL@example.invalid） |
...
3. HTMLメールとしてフォーマット
4. `REDACTED-CLIENT-EMAIL@example.invalid` に自動送信
...
## 11. 権限移譲チェックリスト（BtoB事業会社A社 様）

### BiqQuery アクセス確認
- [x] `REDACTED-CLIENT-EMAIL@example.invalid` にBigQuery閲覧権限付与済み（2026-04-22）
- [ ] GCPコンソール（console.cloud.google.com）へのログイン確認
- [ ] BigQuery → `staging` / `marts` / `reports` が見えることを確認
- [ ] `reports.rpt_looker_main` のプレビュー確認

### Looker Studio
- [x] `REDACTED-CLIENT-EMAIL@example.invalid` に閲覧権限付与済み（2026-04-22）
- [ ] URLでダッシュボードが開けることを確認
- [ ] 全5ページ（主要分析・ファネル・チャネル・ページ・統合）が表示されることを確認

### Streamlit QAアプリ
- [ ] https://REDACTED-APP.example/ が開けることを確認
- [ ] サイドバーのボタンをクリックして回答が返ることを確認
- [ ] 「今月のセッション数の傾向はどうですか？」でグラフが表示されることを確認

### GTMタグ
- [x] REDACTED-GTM-ID が `example.invalid` に設置済み（2026-04-23 確認済み）
