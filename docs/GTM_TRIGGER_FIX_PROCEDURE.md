> ⚠️ **SUPERSEDED（2026-05-15夜・真因誤認のため廃止）**
> 本書はトリガー条件のミスマッチを真因と仮定していたが、5/15ライブ検証で
> 真因は「Custom HTML の `gtag()` が GTM経由GA4に未到達」と確定。
> **正式な修正手順は [`GTM_FIX_PACKAGE_v2.md`](GTM_FIX_PACKAGE_v2.md) を参照（測定ID G-Y95H1GWBB7 反映済）。**
> 本書は調査経緯の記録として残置。

# GTM トリガー条件 修正手順書（B⑤ 5/15 検証 → 修正実施用）【SUPERSEDED】
...
3. アカウント一覧に「株式会社アテンド」または `ark-hd.co.jp` 関連のアカウントが表示されるか確認

#### ケースA: 表示される（招待済み）

- そのままコンテナ `GTM-5B3L5372` をクリック → **「2. タグ①の修正」へ進む**
...
- ark-hd.co.jp の HTML には `data-tracking` 属性が存在しないため、その条件は**マッチしない**
...
```javascript
var ctaSelectors = [
  '.cta-button',         // ← ark-hd.co.jp に存在しない場合あり
  '.btn-contact',        // ← ark-hd.co.jp に存在しない場合あり
  'a[href*="/contact"]', // ← 最も汎用的・「お問い合わせ」リンクはこれでカバー
  'a[href*="contact.html"]',
  '[data-cta]'           // ← ark-hd.co.jp に存在しない
];
```

ark-hd.co.jp のHTML構造を踏まえると、**`a[href*="/contact"]` だけで90%のCTAクリックは捕捉できる**。
...
3. URL欄に `https://www.ark-hd.co.jp/` を入力 → 「Connect」
4. ark-hd.co.jp のサイトが新タブで開く（Tag Assistant 接続済み）

### 5.2 タグ①の発火確認（スクロール）

1. ark-hd.co.jp のページで下までスクロール
...
1. ark-hd.co.jp のページ内「お問い合わせ」リンク（hero / footer / 中間）をクリック
...
1. `https://www.ark-hd.co.jp/contact/` を開く
...
1. GA4管理画面（プロパティID `386840839`）→ 左メニュー「設定」→「DebugView」
...
- [ ] 弊社担当者アカウントで GTM-5B3L5372 にアクセス可能
...
発火状況確認を進めておりますが、
GTM管理画面（GTM-5B3L5372）の設定内容との突合せが必要になりました。
