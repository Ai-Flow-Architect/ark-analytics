...
example.comのGA4分析データをLooker Studioで可視化する手順です。
...
| テーブル | 用途 |
|---------|------|
| `__ARK_PROJECT__.reports.rpt_looker_main` | メインダッシュボード（KPI+ファネル統合） |
| `__ARK_PROJECT__.marts.daily_kpi_summary` | 日次KPIトレンド |
| `__ARK_PROJECT__.marts.channel_kpi_monthly` | チャネル別分析 |
| `__ARK_PROJECT__.marts.page_performance` | ページ別パフォーマンス |
| `__ARK_PROJECT__.marts.conversion_funnel_daily` | ファネル分析 |
...
2. example.comのGoogleアカウント（BQ閲覧権限があるアカウント）でログイン
...
2. 左サイドバーでプロジェクト `__ARK_PROJECT__` を選択
3. 「IAMと管理」→「IAM」
4. 「アクセス権を付与」をクリック
...
1. BQコンソール → `__ARK_PROJECT__` → `marts` データセットを選択
2. 「共有」→「アクセス許可の編集」
...
FROM `__ARK_PROJECT__.reports.rpt_looker_main`
